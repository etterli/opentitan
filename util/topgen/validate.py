...
module_required = {
    ...
    # A split IP keys these by partition; every other instance uses the flat
    # form described here.
...
def validate_reset(top: ConfigT,
    ...
    is_block = isinstance(inst, IpBlock)
    if is_block:
        name = inst.name
    else:
        name = inst['name']
    ...
    # If value is a string, it is taken to be in the power domain of the
    # partition it belongs to (amend_reset_connections normally does this
    ...
    def check_reset_connections(reset_connections, reset_signals, domain):
        err = 0
    ...
    if not is_block:
        # Crossbars are never split and keep the flat form.
        return error + check_reset_connections(
            module['reset_connections'], reset_signals,
            module.get('domain', default_domain))
    ...
    for partition in inst.clocking.partitions:
        ...
        if reset_connections is None:
            continue
        reset_signals = inst.clocking.reset_signals(partition)
        log.info(f"{prefix} {name} {partition} resets are {reset_signals}")
        error += check_reset_connections(
            reset_connections, reset_signals,
            partition_domain(module, partition, default_domain))
...
def validate_clock(top: ConfigT,
    ...
    if not is_block:
        # Crossbars are never split and keep the flat form.
        return error + check_clock_srcs(module['clock_srcs'], clock_signals)
    ...
    for partition in inst.clocking.partitions:
        ...
        if clock_srcs_map is None:
            continue
        error += check_clock_srcs(clock_srcs_map,
                                  inst.clocking.clock_signals(False, partition))
