 ...
 * extra required bn.addvm with a zero vector is performed, so the result is correct Montgomery
 * computation.
 ...
 * @param[in]  dmem[.]: mod32_montg;  The modulus (q) and Montgomery constant (mu) for bn.mulvm(l)
                                      Expected format: q @ [31:0], mu @ [63:32]
 ...
 *********************************/
/* complete WSR for MOD with the modulus (q) and the Montgomery constant (mu) at the following bits:
 * `q @ MOD[31:0]`, `mu @ MOD[63:32]`
 */
.globl mod32_montg
.balign 32
mod32_montg:
