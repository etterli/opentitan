...
static status_t run_mldsa87_sign(const mldsa87_sign_testvector_t *vector) {
  ...
  /* otcrypto_const_byte_buf_t rnd = OTCRYPTO_MAKE_BUF( */
  /*     otcrypto_const_byte_buf_t, vector->rnd, kOtcryptoMldsa87RndBytes); */

  uint32_t sig_data[kOtcryptoMldsa87SigWords];
  otcrypto_word32_buf_t sig = OTCRYPTO_MAKE_BUF(otcrypto_word32_buf_t, sig_data, kOtcryptoMldsa87SigWords);
  CHECK_STATUS_OK(otcrypto_mldsa87_sign(
      &sk, &msg, &ctx, vector->hash_mode, vector->sign_mode, &sig));

  CHECK_ARRAYS_EQ(kSigNistAcvp61, sig_data, kOtcryptoMldsa87SigWords);
