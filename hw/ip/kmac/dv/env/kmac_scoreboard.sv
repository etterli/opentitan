...
class kmac_scoreboard extends cip_base_scoreboard #(
  ...
  // output digest from KMAC_APP intf (256 bits each, static interfaces)
  bit [kmac_pkg::AppDigestW-1:0] kmac_app_digest_share0;
  bit [kmac_pkg::AppDigestW-1:0] kmac_app_digest_share1;

  // Assembled digest shares for dynamic app interface
      ...
      `DV_SPINWAIT_EXIT(
          forever begin
            ...
            if (`KMAC_APP_VALID_TRANS(AppKeymgr)) begin
              app_mode = AppKeymgr;
              strength = APP_CFG[app_mode].KeccakStrength;
              ...
              app_mode = AppOtbn;
              // Strength is encoded in the config
  ...
  virtual task process_kmac_app_req_fifo();
    ...
    forever begin
        ...
        `DV_SPINWAIT_EXIT(
            // For AppDynamic, the first push_pull item is the config beat
            // (mode/strength encoded in data_s0). Discard it before collecting message.
            if (app_mode == AppOtbn) begin
              kmac_app_req_fifo[app_mode].get(kmac_app_block_item);
              `uvm_info(`gfn, "AppOtbn: discarded config beat", UVM_HIGH)
            end
            // For AppDynamic the req fifo also contains post-message digest request
            // beats; stop appending to kmac_app_msg after the message last beat.
            begin
              bit otbn_msg_done = 0;
              forever begin
                kmac_app_req_fifo[app_mode].get(kmac_app_block_item);
                ...
                if (!otbn_msg_done) begin
                  ...
                  if (kmac_app_last && app_mode == AppOtbn) otbn_msg_done = 1;
                end
              end
            end
            ,
          ...
          forever begin
            ...
            `DV_SPINWAIT_EXIT(
                ...
                if (app_intf_err) begin
                  ...
                  // Dynamic mode: assemble byte array from 64-bit chunks.
                  begin
  ...
  virtual function void check_digest();
    ...
    app_mode_e cfg_app_mode =
        APP_CFG[app_mode].Type == AppStatic ? APP_CFG[app_mode].Mode :
    ...
    sha3_pkg::keccak_strength_e cfg_app_strength =
        APP_CFG[app_mode].Type == AppStatic ? APP_CFG[app_mode].KeccakStrength :
    ...
    if (in_kmac_app) begin
      if (APP_CFG[app_mode].Type == AppDynamic) begin
        ...
        // xof_en is 1 for Shake and CShake modes in dynamic app interfaces.
        xof_en = (actual_hash_mode == sha3_pkg::Shake || actual_hash_mode == sha3_pkg::CShake);
      end else begin
    ...
    case (actual_hash_mode)
      ...
      ///////////
      sha3_pkg::Sha3: begin
        // SHA3 DPI functions produce a fixed-size digest. For AppDynamic partial reads (e.g.
        // requesting 2 of 4 SHA3-256 chunks), allocate a full-sized buffer and truncate.
        int full_sha3_bytes;
        ...
        endcase
        for (int i = 0; i < total_len_bytes; i++) dpi_digest[i] = sha3_full_buf[i];
      end
      ///////////
      // SHAKE //
      ///////////
      sha3_pkg::Shake: begin
        case (actual_strength)
          sha3_pkg::L128: begin
            digestpp_dpi_pkg::c_dpi_shake128(msg_arr, msg_arr.size(), total_len_bytes,
                                             xof_en, dpi_digest);
          end
          sha3_pkg::L256: begin
            digestpp_dpi_pkg::c_dpi_shake256(msg_arr, msg_arr.size(), total_len_bytes,
                                             xof_en, dpi_digest);
          end
          default: begin
            `uvm_fatal(`gfn,
      ...
      sha3_pkg::CShake: begin
        ...
        if (kmac_en) begin
          ...
          case (actual_strength)
            ...
            default: begin
              `uvm_fatal(`gfn, $sformatf("strength[%0s] is not allowed for kmac", actual_strength.name()))
            end
          endcase
        end else begin
          ...
          case (actual_strength)
            sha3_pkg::L128: begin
              digestpp_dpi_pkg::c_dpi_cshake128(msg_arr, fname, custom_str, msg_arr.size(),
                                                total_len_bytes, xof_en, dpi_digest);
            end
            sha3_pkg::L256: begin
            ...
            default: begin
              `uvm_fatal(`gfn, $sformatf("strength[%0s] is not allowed for cshake", actual_strength.name()))
            end
          endcase
        end
      end
    ...
    case (actual_hash_mode)
      ...
      sha3_pkg::Sha3: begin
        case (actual_strength)
          ...
          default: begin
            `uvm_fatal(`gfn, $sformatf("strength[%0s] is not allowed for sha3", actual_strength.name()))
          end
        endcase
      end
