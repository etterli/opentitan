...
package kmac_pkg;
  ...
  typedef struct packed {
    ...
    logic EnUnsupportedModeStrength;
    ...
    //
    // For static interfaces, if PrefixMode is set, the Prefix will be used for both cSHAKE and
    // KMAC operations. If reset, the CSR value is used.
    //
    // For dynamic interfaces, PrefixMode is set depending on the selected mode.
    // If the mode is cSHAKE, the CSR prefix is used.
    // If the mode is KMAC, the compile-time value is used.
    logic PrefixMode;

    // If set, the app interface will automatically trigger a RUN command once it has pushed the
    // full rate on the response channel. If reset, no squeeze can be performed at all. Usually
    // enabled for SHAKE and cSHAKE and disabled for SHA3 and KMAC. Has no effect on static
    // interfaces.
    logic EnXof;
  } app_config_t;
  ...
  parameter app_config_t AppCfgDefault = '{
    ...
    EnXof: 1'b0,
    EnUnsupportedModeStrength: 1'b0
  };
  ...
  typedef enum logic [AppStateWidth-1:0] {
    ...
    StErrorAwaitFinish   = 10'b0101101000,
