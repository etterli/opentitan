...
#(
  ...
  // TODO: Do we have to harden fifo_bypass_i with MuBi? Otherwise it is easy to skip / manipulate
  //       messages.
  ...
  // TODO: Do we need to delay process_o by one cycle when bypassing to have the same minimal delay
  // as when flushing the packer and FIFO?
...
`ifdef INC_ASSERT
  ...
  // Assert that bypass control signal is stable after the first messages was handshaked until FIFO
  // is cleared.
  ...
  // fifo_bypass_i must remain stable from first data entry until all data is flushed out or passed
