...
class Dmem:
    ...
    def __init__(self) -> None:
        ...
        # Each location has a flag to indicate whether the DMEM is valid or has
        # been invalidated to simulate an integrity error.
        num_words = dmem_size // 4
        self.data: List[Tuple[Optional[int], bool]] = [(None, True)] * num_words
        ...
        for idx32, (vld, u32) in enumerate(struct.iter_unpack('<BI', data)):
            if vld not in [0, 1]:
                raise ValueError('The validity byte for 32-bit word {} '
                                 ...
                                 .format(idx32, vld))
            self.data[idx32 + word_offset] = (u32 if vld else None, True)
    ...
    def load_u256(self, addr: int) -> Optional[int]:
        ...
        for i in range(256 // 32):
            rd_data, valid_i = self.load_u32(addr + 4 * i)
            ret_data = ret_data | (rd_data << (i * 32))
            valid = valid and valid_i
    ...
    def load_u32(self, addr: int) -> Tuple[Optional[int], bool]:
