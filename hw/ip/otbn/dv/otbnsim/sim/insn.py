...
class BNADDV(BnVecVecAdd):
    ...
    def execute(self, state: OTBNState) -> None:
        ...
        if size not in self.supported_elens:
            state.stop_at_end_of_cycle(ErrBits.ILLEGAL_INSN)
            return None
...
class BNUNPK(OTBNInsn):
    ...
    def __init__(self, raw: int, op_vals: Dict[str, int]):
        ...
        self.wrs1 = op_vals['wrs1']
        self.wrs2 = op_vals['wrs2']
        self.shift = op_vals['shift']

    def execute(self, state: OTBNState) -> None:
        vec_a = state.wdrs.get_reg(self.wrs1).read_unsigned()
        vec_b = state.wdrs.get_reg(self.wrs2).read_unsigned()

        combined = (vec_a << 256) | (vec_b)  # Concatenate to 512 bits
        assert 0 <= combined < (1 << 512)
        shifted = combined >> self.shift

        # This instruction always performs 24-bit to 32-bit unpacking
        unpacked = 0
        mask_24bit = (1 << 24) - 1
        for elem in range(256 // 32):
            chunk = (shifted >> (elem * 24)) & mask_24bit
...
class BNPACK(OTBNInsn):
    ...
    def execute(self, state: OTBNState) -> None:
        ...
        # This instruction always performs 32-bit to 24-bit packing
        mask_24bit = (1 << 24) - 1

        # Extract 8 24-bit elements into 192-bit string
        def extract_24b_elems(vec: int) -> int:
            dense = 0
            for elem in range(256 // 32):
                elem_a = (vec >> (elem * 32)) & mask_24bit
                dense = dense | (elem_a << (elem * 24))
            assert 0 <= dense < (1 << 192)
            return dense

        dense_a = extract_24b_elems(vec_a)
        dense_b = extract_24b_elems(vec_b)
