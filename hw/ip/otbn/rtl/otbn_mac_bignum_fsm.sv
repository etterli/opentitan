...
(
  ...
  output mac_bignum_predec_dyn_t predec_dyn_o,
   ...
   * MAC. Any dynamic signal that requires predecoding is generated with this FSM inside the
   * predecoder such that it can be flopped. It is then forwarded to the BN MAC which will compare
   * the signals to the locally generated predecode signals (as expected signals).
   *
   ...
   * execution. See the decoder / predecoder for how these are controlled.
   *
  ...
  always_comb begin
    ...
    predec_dyn_reg                = MacBignumPredecDynDefault;
  ...
  always_comb begin
    ...
    if (is_lane_i) begin
      predec_dyn_o.op_b_elem0_sel = lane_index_i;
      predec_dyn_o.op_b_elem1_sel = lane_index_i;
    end
  end
