 ...
 * multiplication 32b elements and optional modulo reduction. The vector elements are
 * stored in a WDR. The vector is processed in 64b chunks. These chunks can be interpreted as
 * two 32-bit multiplications. For modulo reduction the Montgomery algorithm is implemented.
 ...
 *   return r
 *
 * To save area the the Montgomery algorithm is split over 3 cycles to reuse the multiplier.
 * This introduces two new registers for temporary values. In addition, the final conditional
 * subtraction (if r >= q) is also not implemented in hardware as it can be performed with a
 * bn.addvm instruction. This optimizes area and timing. When processing a vector chunk, the
 ...
 *
 * All three registers can be cleared with either a secure wipe or by overwriting it via the ispr
 * interface. In both cases all three registers are set to the same value.
 */
...
(
  ...
  otbn_vec_multiplier u_vec_multiplier (
    ...
    .elen_ctrl_i(mac_predec_bignum_i.elen_ctrl),
    .result_o   (mul_res)
  );
  ...
  // TODO: Double check that this bit-selecting for the input will not result in any logic.
  // SEC_CM: DATA_REG_SW.SCA
  ...
  // - mul_res_shifted comes from a blanked value (mul_res_pre_shifted) which is only active if
  //   the shifted result is actually used. c_blanked is directly from a blanker.
  // - mul_res_add and acc_add_blanked are already blanked
  ...
  otbn_vec_adder #(
    ...
    .carries_in_i      ('0),
    .use_ext_carry_i   (mac_predec_bignum_i.adder_carry_sel),
    .sum_o             (adder_result),
  ...
  // The module datapath only uses the lower 128 bits of the vectorized adder.
  logic [128+2*32-1:0] unused_adder_result_mod;
  ...
  // Here a regular MUX is sufficient. The mul_res_merger comes directly from a blanker which is
  // only active (passes data through) if we are performing a vectorized multiplication
  // (default or lane). In this case the montg_r_cor is all zero as the input into the reduction
  //  circuit is blanked with u_add_mod_blanker. If we do a modulo multiplication the
  // `montg_r_cor` contains the data but mul_res_merger is blanked as explained.
   ...
   * - Vectorized Multiplication with modulo reduction (32b elements)
   ...
   *     to process one quarter word (i.e. 2x 32b elements). Thus the total duration is
   *     12 cycles.
  ...
  logic [NELEN_MAC-1:0] expected_elen_ctrl;
  ...
  assign expected_elen_ctrl       = operation_i.elen_ctrl;
  ...
  // TODO: Signals controlled by the FSM which should be predecoded
  // - mul_add_en
  // - mul_mod_en
  // - c_add_en
  // - add_mod_en
  ...
  assign predec_error_o = |{expected_op_en           != mac_predec_bignum_i.op_en,
                            ...
                            expected_elen_ctrl       != mac_predec_bignum_i.elen_ctrl,
                            expected_adder_carry_sel != mac_predec_bignum_i.adder_carry_sel,
                            expected_lane_index      != mac_predec_bignum_i.lane_index,
