...
#(
  ...
  // The BN MAC sets certain predecoded signals depending on the internal state. The insn_fetch_en
  // must take priority over the forward request for security reasons if the valid flag of BN MAC
  // is FIed to high prematurely.
  assign mac_bignum_predec_dyn_d =
      insn_fetch_en                      ? mac_bignum_predec_dyn        :
      mac_bignum_predec_dyn_next_valid_i ? mac_bignum_predec_dyn_next_i : mac_bignum_predec_dyn_q;
