 ...
 *
 * The packing and unpacking functionality uses the shifter to combine two WDRs which allows
 * efficient packing and unpacking of 32-bit elements to/from 24-bit wide elements.
 *
...
(
  ...
  // Operand A is only used for BN.RSHI and BN.PACK. For all other instructions
  // alu_bignum_predec_i.shifter_a_en will be 0, resulting in 0 for the upper input.
  assign shifter_in_upper = alu_bignum_predec_i.shift_pack_sel ? shifter_operand_a_blanked
                                                               : operand_a_packed;
  assign shifter_in_lower = alu_bignum_predec_i.shift_pack_sel ? shifter_operand_b_blanked
                                                               : operand_b_packed;

  otbn_vec_shifter u_vec_shifter (
    .shifter_in_upper_i(shifter_in_upper),
    .shifter_in_lower_i(shifter_in_lower),
    .shift_right_i     (alu_bignum_predec_i.shift_right),
  ...
  otbn_vec_adder #(
    ...
    .use_ext_carry_i   ({NVecProc{alu_bignum_predec_i.vec_adder_carry_sel}}),
    .sum_o             (adder_x_res),
  ...
  prim_blanker #(.Width(NVecProc)) u_mod_op_res_sel_carry_y_blanked (
    ...
    .en_i (alu_bignum_predec_i.vec_mod_selector_en),
  ...
  otbn_vec_mod_result_selector u_mod_op_res_selector (
    ...
    .is_subtraction_i(alu_bignum_predec_i.vec_mod_is_subtraction),
  ...
  logic                    expected_vec_adder_carry_sel;
  ...
  assign alu_predec_error_o =
    |{expected_adder_x_en != alu_bignum_predec_i.adder_x_en,
      ...
      expected_vec_adder_carry_sel != alu_bignum_predec_i.vec_adder_carry_sel,
      expected_vec_mod_selector_en != alu_bignum_predec_i.vec_mod_selector_en,
      expected_vec_mod_is_subtraction != alu_bignum_predec_i.vec_mod_is_subtraction,
      ...
      expected_shift_pack_sel != alu_bignum_predec_i.shift_pack_sel,
