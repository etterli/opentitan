...
#(
  ...
  parameter bit          TargetExt        = 1'b0,  // Include Target-side hardware extension?
  ...
  input                     bufq_req_i[TLWordCnt],
  output                    bufq_gnt_o[TLWordCnt],
  input                     bufq_we_i[TLWordCnt],
  input               [3:0] bufq_addr_i[TLWordCnt],
  input     [DataWidth-1:0] bufq_wdata_i[TLWordCnt],
  output                    bufq_rvalid_o[TLWordCnt],
  output    [DataWidth-1:0] bufq_rdata_o[TLWordCnt],
  ...
  input  i3c_targ_bus_obs_t targ_bus_obs_i,
  ...
  prim_flop_2sync #(.Width(2)) u_sync_ctrl(
    .clk_i  (clk_i),
    .rst_ni (rst_ni),
    .d_i    ({ctrl_bus_obs_i.sda[0], ctrl_bus_obs_i.scl}),
  ...
  prim_flop_2sync #(.Width(2)) u_sync_targ(
    .clk_i  (clk_i),
    .rst_ni (rst_ni),
    .d_i    ({targ_bus_obs_i.sda[0], targ_bus_obs_i.scl}),
  ...
  i3c_input_buffers #(.NumSDALanes(NumSDALanes)) u_targ_inbufs (
    ...
    // I3C target inputs.
    .scl_i      (targ_bus_obs_i.scl),
    .sda_i      (targ_bus_obs_i.sda),
  ...
  logic                 targ_buf_rempty[NumTargets];
  logic [FIFODepthW:0]  targ_buf_rused[NumTargets];
  logic [FIFODepthW:0]  targ_buf_ravail[NumTargets];

  always_comb begin
    // Read access to the message buffer for the configured, extant Virtual Targets.
    for (int unsigned t = 0; t < NumTargets; t++) begin
      ...
      targ_buf_rempty[t] = fifo_state[FIFO_TxTarg0 + t].empty;
      targ_buf_rused[t]  = fifo_state[FIFO_TxTarg0 + t].used;
      targ_buf_ravail[t] = fifo_state[FIFO_TxTarg0 + t].avail;
    end
    // Non-extant Virtual Targets.
    for (int unsigned t = NumTargets; t < MaxTargets; t++) begin
      fifo_in[FIFO_TxDTarg0 + t].rready = 1'b0;
      fifo_in[FIFO_TxTarg0  + t].rready = 1'b0;
    end
  end
  ...
  assign hw2reg_o.targ_pio_control.suspended.d  = reg2hw_i.targ_pio_control.suspended.q
                                                | targ_suspend_tx;
  // Suspend IBI Transmission from target; hardware only ever sets the 'suspended' bit.
  ...
  assign hw2reg_o.targ_pio_control.ibi_abort.d = 1'b0;  // Hardware only ever clears the 'abort'.
  ...
  always_comb begin
    for (int unsigned t = 0; t < MaxTargets; t++) begin
      if (t < NumTargets) begin
        ...
        hw2reg_o.targ_info[t].as.d  = targ_act_state_d;
      end else hw2reg_o.targ_info[t] = '0;  // Virtual Target not present.
    end
  end

  // Status of Standby Controller Requests.
  // - Standby Controller functionality is not implemented.
  assign hw2reg_o.stby_cr_status.hj_req_status.d = 1'b0;
  assign hw2reg_o.stby_cr_status.simple_crr_status.d = 2'b00;
  assign hw2reg_o.stby_cr_status.ac_current_own.d = 1'b0;
  ...
  i3c_target #(
    ...
    .hj_request_clear_o   (hw2reg_o.targ_control.hj_request.de),
    ...
    .ibi_rempty_i         (fifo_state[FIFO_IBITarg].empty),
    ...
    // Target Reset Detector request/response.
    .rstdet_req_o         (rstdet_o),
    .rstdet_rsp_i         (rstdet_i),
    ...
    .sink_dwords_left_o   (hw2reg_o.targ_sink_status.length.d),
    .sink_active_o        (hw2reg_o.targ_sink_status.active.d),
    .sink_error_o         (hw2reg_o.targ_sink_status.error.d),

    // Extension hardware.
    .ext_present_o        (hw2reg_o.targ_status.ext_present.d),
    .ext_info_o           (hw2reg_o.targ_status.ext_info.d),
    .ext_reg2hw_i         ('0),
    .ext_hw2reg_o         (),
  ...
  // The number of extant Virtual Targets may be configured; extract their buffer configuration.
  for (genvar t = 0; t < NumTargets; t++) begin : gen_vt_buf_config
    assign fifo_cfg[FIFO_TxTarg0 + t].min  = BufAddrW'(reg2hw_i.targ_txbuf_config[t].min_addr.q);
    assign fifo_cfg[FIFO_TxTarg0 + t].max  = BufAddrW'(reg2hw_i.targ_txbuf_config[t].max_addr.q);
    assign fifo_cfg[FIFO_TxTarg0 + t].size = fifo_size(fifo_cfg[FIFO_TxTarg0 + t]);

    assign fifo_cfg[FIFO_TxDTarg0 + t].min  = BufAddrW'(reg2hw_i.targ_txdesc_config[t].min_addr.q);
    assign fifo_cfg[FIFO_TxDTarg0 + t].max  = BufAddrW'(reg2hw_i.targ_txdesc_config[t].max_addr.q);
    assign fifo_cfg[FIFO_TxDTarg0 + t].size = fifo_size(fifo_cfg[FIFO_TxDTarg0 + t]);
  end
  for (genvar t = NumTargets; t < MaxTargets; t++) begin : gen_no_vt_buf_config
    assign fifo_cfg[FIFO_TxTarg0  + t].min  = 'b0;
    assign fifo_cfg[FIFO_TxTarg0  + t].max  = 'b0;
    assign fifo_cfg[FIFO_TxTarg0  + t].size = 'b0;
    assign fifo_cfg[FIFO_TxDTarg0 + t].min  = 'b0;
    assign fifo_cfg[FIFO_TxDTarg0 + t].max  = 'b0;
    assign fifo_cfg[FIFO_TxDTarg0 + t].size = 'b0;
  end
  ...
  // The number of extant Virtual Targets may be configured; group their diagnostic state here.
  for (genvar t = 0; t < NumTargets; t++) begin : gen_targ_tx
    assign hw2reg_o.targ_txbuf_state[t].wptr.d  = fifo_state[FIFO_TxTarg0  + t].wptr;
    assign hw2reg_o.targ_txdesc_state[t].wptr.d = fifo_state[FIFO_TxDTarg0 + t].wptr;
    assign hw2reg_o.targ_txbuf_state[t].rptr.d  = fifo_state[FIFO_TxTarg0  + t].rptr;
    assign hw2reg_o.targ_txdesc_state[t].rptr.d = fifo_state[FIFO_TxDTarg0 + t].rptr;
    assign hw2reg_o.targ_txbuf_state[t].full.d  = fifo_state[FIFO_TxTarg0  + t].full;
    assign hw2reg_o.targ_txdesc_state[t].full.d = fifo_state[FIFO_TxDTarg0 + t].full;
    assign hw2reg_o.targ_txbuf_state[t].pre.d   = fifo_state[FIFO_TxTarg0  + t].pre;
    assign hw2reg_o.targ_txdesc_state[t].pre.d  = fifo_state[FIFO_TxDTarg0 + t].pre;
  end
  for (genvar t = NumTargets; t < MaxTargets; t++) begin : gen_no_targ_tx
    assign hw2reg_o.targ_txbuf_state[t].wptr.d  = 'b0;
    assign hw2reg_o.targ_txdesc_state[t].wptr.d = 'b0;
    assign hw2reg_o.targ_txbuf_state[t].rptr.d  = 'b0;
    assign hw2reg_o.targ_txdesc_state[t].rptr.d = 'b0;
    assign hw2reg_o.targ_txbuf_state[t].full.d  = 1'b0;
    assign hw2reg_o.targ_txdesc_state[t].full.d = 1'b0;
    assign hw2reg_o.targ_txbuf_state[t].pre.d   = 1'b0;
    assign hw2reg_o.targ_txdesc_state[t].pre.d  = 1'b0;
  end
  ...
  always_comb begin
    // Writing to the message buffer via HCI ports.
    fifo_in[FIFO_CmdQ].wvalid      = hci_wvalid[HCI_Command];
    fifo_in[FIFO_CmdQ].wdata       = hci_wdata;
    fifo_in[FIFO_TxBuf].wvalid     = hci_wvalid[HCI_XferData];
    fifo_in[FIFO_TxBuf].wdata      = hci_wdata;
    fifo_in[FIFO_IBIDTarg].wvalid  = tti_wvalid[TTI_IBIDesc];
    fifo_in[FIFO_IBIDTarg].wdata   = tti_wdata;
    fifo_in[FIFO_IBITarg].wvalid   = tti_wvalid[TTI_IBIData];
    fifo_in[FIFO_IBITarg].wdata    = tti_wdata;

    // Writing to the message buffer via TTI ports.
    for (int unsigned t = 0; t < NumTargets; t++) begin
      fifo_in[FIFO_TxDTarg0 + t].wvalid = tti_wvalid[TTI_Tx0Desc + t];
      fifo_in[FIFO_TxDTarg0 + t].wdata  = tti_wdata;
      fifo_in[FIFO_TxTarg0  + t].wvalid = tti_wvalid[TTI_Tx0Data + t];
      fifo_in[FIFO_TxTarg0  + t].wdata  = tti_wdata;
    end
    for (int unsigned t = NumTargets; t < MaxTargets; t++) begin
      fifo_in[FIFO_TxDTarg0 + t].wvalid = 1'b0;
      fifo_in[FIFO_TxDTarg0 + t].wdata  = '0;
      fifo_in[FIFO_TxTarg0  + t].wvalid = 1'b0;
      fifo_in[FIFO_TxTarg0  + t].wdata  = '0;
    end

    // Reading from the message buffer via HCI ports.
    fifo_in[FIFO_RspQ].rready      = hci_rready[HCI_Response];
    fifo_in[FIFO_RxBuf].rready     = hci_rready[HCI_XferData];
    // IBI_PORT is implemented as two queues, one carrying the IBI payload data and the other
    // holding IBI Status Descriptors.
    fifo_in[FIFO_IBIQ].rready      = hci_rready[HCI_IBI] & !ibi_status_desc;
    fifo_in[FIFO_IBIStD].rready    = hci_rready[HCI_IBI] &  ibi_status_desc;

    // Reading from the message buffer via TTI ports.
    fifo_in[FIFO_RxDTarg].rready   = tti_rready[TTI_RxDesc];
    fifo_in[FIFO_RxTarg].rready    = tti_rready[TTI_RxData];
    fifo_in[FIFO_AsyncTarg].rready = tti_rready[TTI_AsyncEvt];
  end
  ...
  i3c_queues #(
    ...
    .buf_wready_i({fifo_out[FIFO_RxDTarg].wready,
                   ...
                   fifo_out[FIFO_TxDTarg2].wready,
                   fifo_out[FIFO_TxDTarg3].wready,
                   fifo_out[FIFO_TxTarg0].wready,
                   fifo_out[FIFO_TxTarg1].wready,
                   fifo_out[FIFO_TxTarg2].wready,
                   fifo_out[FIFO_TxTarg3].wready}),
    .buf_full_i  ({fifo_state[FIFO_RxDTarg].full,
                   ...
                   // Qualifying the `full` signals for the Virtual Targets both allows synthesis to
                   // strip out unused logic and prevents system-side bus accesses from locking up;
                   // writes will complete immediately.
                   fifo_state[FIFO_TxDTarg0].full | (NumTargets < 1),
                   fifo_state[FIFO_TxDTarg1].full | (NumTargets < 2),
                   fifo_state[FIFO_TxDTarg2].full | (NumTargets < 3),
                   fifo_state[FIFO_TxDTarg3].full | (NumTargets < 4),
                   fifo_state[FIFO_TxTarg0].full  | (NumTargets < 1),
                   fifo_state[FIFO_TxTarg1].full  | (NumTargets < 2),
                   fifo_state[FIFO_TxTarg2].full  | (NumTargets < 3),
                   fifo_state[FIFO_TxTarg3].full  | (NumTargets < 4)}),
    ...
    .buf_rvalid_i({fifo_out[FIFO_RxDTarg].rvalid,
                   ...
                   fifo_out[FIFO_TxDTarg2].rvalid,
                   fifo_out[FIFO_TxDTarg3].rvalid,
                   fifo_out[FIFO_TxTarg0].rvalid,
                   fifo_out[FIFO_TxTarg1].rvalid,
                   fifo_out[FIFO_TxTarg2].rvalid,
                   fifo_out[FIFO_TxTarg3].rvalid}),
    .buf_rdata_i ({fifo_out[FIFO_RxDTarg].rdata,
                   ...
                   fifo_out[FIFO_TxDTarg2].rdata,
                   fifo_out[FIFO_TxDTarg3].rdata,
                   fifo_out[FIFO_TxTarg0].rdata,
                   fifo_out[FIFO_TxTarg1].rdata,
                   fifo_out[FIFO_TxTarg2].rdata,
                   fifo_out[FIFO_TxTarg3].rdata}),
    .buf_empty_i ({fifo_state[FIFO_RxDTarg].empty,
                   ...
                   // Qualifying the `empty` signals for the Virtual Targets both allows synthesis
                   // to strip out unused logic and prevents system-side bus accesses from locking
                   // up; writes will complete immediately.
                   fifo_state[FIFO_TxDTarg0].empty | (NumTargets < 1),
                   fifo_state[FIFO_TxDTarg1].empty | (NumTargets < 2),
                   fifo_state[FIFO_TxDTarg2].empty | (NumTargets < 3),
                   fifo_state[FIFO_TxDTarg3].empty | (NumTargets < 4),
                   fifo_state[FIFO_TxTarg0].empty  | (NumTargets < 1),
                   fifo_state[FIFO_TxTarg1].empty  | (NumTargets < 2),
                   fifo_state[FIFO_TxTarg2].empty  | (NumTargets < 3),
                   fifo_state[FIFO_TxTarg3].empty  | (NumTargets < 4)}),
  ...
  // When software has direct access to the entire message buffer we can map at most 4KiB into
  // the address space, so collect any extra address bits from the register in case the configured
  // message buffer size is larger.
  ...
  i3c_intr #(.Width($bits(i3c_targ_intr_t)), .EdgeTrig('0)) u_targ_intr (
    ...
    .status_en_i({reg2hw_i.targ_intr_status_enable.te_stat_en.q,
                  ...
                  reg2hw_i.targ_intr_status_enable.async_evt_ovf_stat_en.q,
                  reg2hw_i.targ_intr_status_enable.rx_buffer_ovf_stat_en.q,
    ...
    .status_de_o({hw2reg_o.targ_intr_status.te_stat.de,
                  ...
                  hw2reg_o.targ_intr_status.async_evt_ovf_stat.de,
                  hw2reg_o.targ_intr_status.rx_buffer_ovf_stat.de,
    ...
    .status_d_o ({hw2reg_o.targ_intr_status.te_stat.d,
                  ...
                  hw2reg_o.targ_intr_status.async_evt_ovf_stat.d,
                  hw2reg_o.targ_intr_status.rx_buffer_ovf_stat.d,
    ...
    .status_i   ({reg2hw_i.targ_intr_status.te_stat.q,
                  ...
                  reg2hw_i.targ_intr_status.async_evt_ovf_stat.q,
                  reg2hw_i.targ_intr_status.rx_buffer_ovf_stat.q,
    ...
    .signal_en_i({reg2hw_i.targ_intr_signal_enable.te_signal_en.q,
                  ...
                  reg2hw_i.targ_intr_signal_enable.async_evt_ovf_signal_en.q,
                  reg2hw_i.targ_intr_signal_enable.rx_buffer_ovf_signal_en.q,
  ...
  localparam bit [11:0] BASE_OFFSET = I3C_HCI_VERSION_OFFSET;
  ...
  assign hw2reg_o.hc_capabilities.scheduled_commands_en.d = 1'b0;
  ...
  assign hw2reg_o.hc_capabilities.standby_cr_cap.d        = SecondaryCtrl;
  ...
  // Addressing offsets.
  assign hw2reg_o.pio_section_offset.d              = 16'(I3C_HCI_PORTS_OFFSET - BASE_OFFSET);
  assign hw2reg_o.dat_section_offset.entry_size.d   = 'h0;
  assign hw2reg_o.dat_section_offset.table_size.d   = 7'(NumDATEntries);
  assign hw2reg_o.dat_section_offset.table_offset.d = 16'(I3C_DAT_OFFSET - BASE_OFFSET);
  ...
  assign hw2reg_o.dct_section_offset.table_offset.d = 16'(I3C_DCT_OFFSET - BASE_OFFSET);
  ...
  assign hw2reg_o.ext_caps_section_offset.d         = 16'(I3C_ID_EXTCAP_HEADER_OFFSET -
                                                          BASE_OFFSET);
  ...
  for (genvar t = 0; t < MaxTargets; t++) begin : gen_targ_lens
    if (t < NumTargets) begin : gen_vt_lens
      assign hw2reg_o.targ_rw_len[t].mwl.de = targ_mwl_de[t];
      assign hw2reg_o.targ_rw_len[t].mrl.de = targ_mrl_de[t];
      assign hw2reg_o.targ_ibi_len[t].de    = targ_ibi_de[t];
      assign hw2reg_o.targ_rw_len[t].mwl.d  = targ_mwl_d;
      assign hw2reg_o.targ_rw_len[t].mrl.d  = targ_mrl_d;
      assign hw2reg_o.targ_ibi_len[t].d     = targ_ibi_d;
    end else begin : gen_no_vt_lens
      assign hw2reg_o.targ_rw_len[t].mwl.de = 1'b0;
      assign hw2reg_o.targ_rw_len[t].mrl.de = 1'b0;
      assign hw2reg_o.targ_ibi_len[t].de    = 1'b0;
      assign hw2reg_o.targ_rw_len[t].mwl.d  = 16'b0;
      assign hw2reg_o.targ_rw_len[t].mrl.d  = 16'b0;
      assign hw2reg_o.targ_ibi_len[t].d     = 8'b0;
    end
  end
  // Present Group Addressing state in the TTI registers.
  for (genvar grp = 0; grp < MaxGroups; grp++) begin : gen_targ_groups
  ...
  assign hw2reg_o.reset_det_status.active.d      = rstdet_i.active;
  assign hw2reg_o.reset_det_status.wake_up.de    = 1'b1;
  assign hw2reg_o.reset_det_status.wake_up.d     = rstdet_i.wake_up_det;
  assign hw2reg_o.reset_det_status.rst_periph.de = 1'b1;
  assign hw2reg_o.reset_det_status.rst_periph.d  = rstdet_i.peri_rst_det;
  assign hw2reg_o.reset_det_status.rst_target.de = 1'b1;
  assign hw2reg_o.reset_det_status.rst_target.d  = rstdet_i.targ_rst_det;

  // Vendor Specific (Extended Capability).
  // - Target Extension.
  assign hw2reg_o.targext_extcap_header.cap_length.d = I3C_TARGEXT_EXTCAP_HEADER_CAP_LENGTH_RESVAL;
  assign hw2reg_o.targext_extcap_header.cap_id.d     = I3C_TARGEXT_EXTCAP_HEADER_CAP_ID_RESVAL;

  // Termination of Extended Capability linked-list.
  assign hw2reg_o.term_extcap_header.cap_length.d = I3C_TERM_EXTCAP_HEADER_CAP_LENGTH_RESVAL;
  assign hw2reg_o.term_extcap_header.cap_id.d     = I3C_TERM_EXTCAP_HEADER_CAP_ID_RESVAL;

  // Target clearing of Hot-Join request, once acknowledged by the Active Controller.
  assign hw2reg_o.targ_control.hj_request.d = 1'b0;
