...
#(
  ...
  // Include the half-cycle SCL extension for additional IP clock frequency support?
  parameter bit                     HalfCycleScl     = 1'b0,
  // Include Target-side Extension hardware?
  ...
  input  prim_ram_1p_pkg::ram_1p_cfg_req_t  dat_cfg_i,
  output prim_ram_1p_pkg::ram_1p_cfg_rsp_t  dat_cfg_o,
  input  prim_ram_1p_pkg::ram_1p_cfg_req_t  dct_cfg_i,
  output prim_ram_1p_pkg::ram_1p_cfg_rsp_t  dct_cfg_o,
  input                                     mbist_en_i,
  input                                     scan_clk_i,
  input                                     scan_rst_ni,
  ...
  // Note: `I3C_BUFFER_SIZE` is in bytes, not bus words.
  ...
  logic  [DataWidth/8-1:0] sw_buf_wmask;
  logic                    sw_buf_rvalid;
  ...
  logic                   bufq_req[TLWordCnt];
  logic                   bufq_gnt[TLWordCnt];
  logic                    bufq_we[TLWordCnt];
  logic           [3:0]  bufq_addr[TLWordCnt];
  logic [DataWidth-1:0] bufq_wdata[TLWordCnt];
  logic [DataWidth-1:0] bufq_rdata[TLWordCnt];
  logic                bufq_rvalid[TLWordCnt];

  for (genvar t = 0; t < TLWordCnt; t++) begin : gen_tlul_win
    tlul_adapter_sram #(
      .SramAw(4),
      .SramDw(DataWidth),
      .Outstanding(2),
  ...
  if (SWDirectMsgBuf) begin : gen_swdirect
    // Group the individual write strobes into bytes.
    logic [DataWidth-1:0] sw_buf_wmask_full;
    for (genvar b = 0; b < DataWidth/8; b++) begin : gen_sw_wmask_full
      assign sw_buf_wmask[b] = |sw_buf_wmask_full[b*8 +: 8];
    end
    ...
    assign {sw_buf_req, sw_buf_we, sw_buf_addr, sw_buf_wdata, sw_buf_wmask} = '0;
