...
interface kmac_app_intf (input clk, input rst_n);
  ...
  // {rsp_digest_share0, rsp_digest_share1, rsp_error, rsp_finished}.
  ...
  assign rsp_data_if.h_data = (if_mode == dv_utils_pkg::Host) ?
      {rsp_digest_share0, rsp_digest_share1, rsp_error, rsp_finished} : 'z;
  ...
  assign rsp_valid                  = (if_mode == dv_utils_pkg::Device) ?
      rsp_data_if.valid : 'z;
  assign {rsp_digest_share0, rsp_digest_share1, rsp_error, rsp_finished} =
  ...
  `ASSERT(DoneAssertAfterLast_A,
    (kmac_data_req.last && kmac_data_req.req_valid && kmac_data_rsp.req_ready) |=>
