...
module ast_part_primary #(
  ...
  input ast_pkg::second_to_prim_t second_to_prim_i,
  output ast_pkg::prim_to_second_t prim_to_second_o,
...
assign rst_sys_clk_n = second_to_prim_i.pwr.vcmain_pok_por && second_to_prim_i.pwr.vcc_pok;
assign rst_io_clk_n  = second_to_prim_i.pwr.vcmain_pok_por && second_to_prim_i.pwr.vcc_pok;
assign rst_usb_clk_n = second_to_prim_i.pwr.vcmain_pok_por && second_to_prim_i.pwr.vcc_pok;

assign clk_sys_pd_n = second_to_prim_i.scan_mode || !second_to_prim_i.clk_osc.deep_sleep;
assign clk_io_pd_n  = second_to_prim_i.scan_mode || !second_to_prim_i.clk_osc.deep_sleep;
assign clk_usb_pd_n = second_to_prim_i.scan_mode || !second_to_prim_i.clk_osc.deep_sleep;
...
prim_flop_2sync #(
  ...
  .clk_i ( clk_sys ),
  .rst_ni ( second_to_prim_i.pwr.vcmain_pok_por ),
  .d_i ( 1'b1 ),
  .q_o ( vcmain_pok_por_sys )
);

assign rst_src_sys_n = second_to_prim_i.scan_mode ? second_to_prim_i.scan_reset_n :
                                                    vcmain_pok_por_sys;
...
assign clks_byp_second_to_prim = second_to_prim_i.clks_byp;
...
assign prim_to_second_o.regal_we = reg2hw.regal.qe;
...
always_ff @( posedge clk_ast_tlul_i, negedge regal_rst_n ) begin
  ...
  end else if ( prim_to_second_o.regal_we ) begin
    regal       <= regal_di;
...
sys_clk u_sys_clk (
  ...
  .clk_src_sys_en_i ( second_to_prim_i.pwrmgr_req.clk_src_sys_en ),
  .clk_sys_pd_ni ( clk_sys_pd_n ),
  .rst_sys_clk_ni ( rst_sys_clk_n ),
  .vcore_pok_h_i ( second_to_prim_i.pwr.vcmain_pok_h ),
  .scan_mode_i ( second_to_prim_i.scan_mode ),
  .sys_osc_cal_i ( second_to_prim_i.sys_io_osc_cal ),
`ifdef AST_BYPASS_CLK
...
io_clk u_io_clk (
  ...
  .clk_src_io_en_i ( second_to_prim_i.pwrmgr_req.clk_src_io_en ),
  .scan_mode_i ( second_to_prim_i.scan_mode ),
  .io_osc_cal_i ( second_to_prim_i.sys_io_osc_cal ),
`ifdef AST_BYPASS_CLK
...
usb_clk u_usb_clk (
  ...
  .clk_src_usb_en_i ( second_to_prim_i.pwrmgr_req.clk_src_usb_en ),
  .usb_ref_val_i ( second_to_prim_i.clk_osc.usb_ref_val ),
  .usb_ref_pulse_i ( second_to_prim_i.clk_osc.usb_ref_pulse ),
  .clk_ast_usb_i ( clk_ast_usb_i ),
  .rst_ast_usb_ni ( rst_ast_usb_ni ),
  .scan_mode_i ( second_to_prim_i.scan_mode ),
  .usb_osc_cal_i ( second_to_prim_i.usb_osc_cal ),
`ifdef AST_BYPASS_CLK
...
ast_clks_byp_primary u_ast_clks_byp_primary (
  .vcmain_pok_i ( second_to_prim_i.pwr.vcmain_pok_h ),
  .vcmain_pok_por_i ( second_to_prim_i.pwr.vcmain_pok_por ),
  .deep_sleep_i ( second_to_prim_i.clk_osc.deep_sleep ),
  .scan_mode_i ( second_to_prim_i.scan_mode ),
  .scan_reset_ni ( second_to_prim_i.scan_reset_n ),
  .clk_ast_tlul_i ( second_to_prim_i.clk_rst.clk_ast_tlul ),
  .dft_clks_byp_i ( 1'b0 ),
...
`endif
  ...
  .second_to_prim_i ( clks_byp_second_to_prim ),
  .prim_to_second_o ( clks_byp_prim_to_second ),
  .io_clk_byp_ack_o ( io_clk_byp_ack_o ),
  .all_clk_byp_ack_o ( all_clk_byp_ack_o ),
  .force_scan_reset_o ( ),
  .clk_src_sys_o ( clk_src_sys ),
  .clk_src_sys_val_o ( prim_to_second_o.pwrmgr_rsp.clk_src_sys_val ),
  .clk_src_io_o ( clk_src_io ),
  .clk_src_io_val_o ( prim_to_second_o.pwrmgr_rsp.clk_src_io_val ),
  .clk_src_io_48m_o ( clk_src_io_48m_o ),
  .clk_src_usb_o ( clk_src_usb ),
  .clk_src_usb_val_o ( prim_to_second_o.pwrmgr_rsp.clk_src_usb_val )
);
...
ast_entropy u_entropy (
  ...
  .clk_src_sys_val_i ( prim_to_second_o.pwrmgr_rsp.clk_src_sys_val ),
  .clk_src_sys_jen_i ( prim_mubi_pkg::mubi4_test_true_loose(clk_src_sys_jen) ),
  .entropy_req_o ( edn_o )
);
...
rng #(
  ...
  .clk_i ( second_to_prim_i.clk_rst.clk_ast_tlul ),
  .rst_ni ( second_to_prim_i.clk_rst.rst_ast_tlul_n ),
  .clk_ast_rng_i ( second_to_prim_i.clk_rst.clk_ast_rng ),
  .rst_ast_rng_ni ( second_to_prim_i.clk_rst.rst_ast_rng_n ),
  .rng_en_i ( rng_en_i ),
...
// Memory configuration
///////////////////////////////////////
assign mem_cfg_o                 = second_to_prim_i.mem_cfg_req;
assign prim_to_second_o.mem_cfg_rsp = mem_cfg_i;

///////////////////////////////////////
// Output Assignments
///////////////////////////////////////
// Clock bypass interface to secondary partition
assign prim_to_second_o.clks_byp = clks_byp_prim_to_second;

// Alert source from TLUL integrity error
assign prim_to_second_o.ot0_alert_src = '{p: intg_err, n: ~intg_err};

`ASSERT_KNOWN(ClkSrcIoKnownO_A, clk_src_io_o, 1, second_to_prim_i.pwr.vcmain_pok_h)
`ASSERT_KNOWN(ClkSrcIo48mKnownO_A, clk_src_io_48m_o, clk_src_io_o, rst_io_clk_n)
`ASSERT_KNOWN(ClkSrcSysKnownO_A, clk_src_sys_o, 1, second_to_prim_i.pwr.vcmain_pok_h)
`ASSERT_KNOWN(ClkSrcUsbKnownO_A, clk_src_usb_o, 1, second_to_prim_i.pwr.vcmain_pok_h)
//
// Alert assertions for reg_we onehot check
`ASSERT_PRIM_REG_WE_ONEHOT_ERROR_TRIGGER_ERR(RegWeOnehot_A,
   u_reg, prim_to_second_o.ot0_alert_src.p, , , clk_ast_tlul_i, rst_ast_tlul_ni)
// RNG
`ASSERT_KNOWN(RngBKnownO_A, rng_b_o, second_to_prim_i.clk_rst.clk_ast_rng,
              second_to_prim_i.clk_rst.rst_ast_rng_n)
`ASSERT_KNOWN(RngValKnownO_A, rng_val_o, second_to_prim_i.clk_rst.clk_ast_rng,
              second_to_prim_i.clk_rst.rst_ast_rng_n)
// ES
...
// Intra-IP
// TODO: Is this the correct clock?
`ASSERT_KNOWN(PrimaryToSecondaryKnownO_A, prim_to_second_o, clk_ast_tlul_i, rst_ast_tlul_ni)
...
// Memory configuration
`ASSERT_KNOWN(MemCfgKnownO_A, mem_cfg_o, clk_ast_tlul_i, second_to_prim_i.pwr.vcaon_pok)
