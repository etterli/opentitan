...
typedef struct packed {
  ...
  typedef struct packed {
    ...
    // Clock source enables from pwrmgr (received on the pwr_ast port in the
    // secondary partition and forwarded to the sys/io/usb oscillators here).
    logic core_clk_en;
    logic io_clk_en;
    logic usb_clk_en;
  ...
  typedef struct packed {
    ...
    // Clock source valids returned to pwrmgr (produced by the sys/io/usb
    // oscillators here and packed into the pwr_ast response in the secondary).
    logic core_clk_val;
    logic io_clk_val;
    logic usb_clk_val;
